...
itemArray xs@(y:ys) = 
    ...
    let list = Item {prev = length xs, value = y, next = 1}: helper xs 1 in 
        newListArray (0, length list-1) list where
            helper [x] i = [Item {prev = i-1, value = x, next = 0}]
            helper (x:xs) i = 
                let elem = Item {prev = i-1, value = x, next = i+1} in
                elem:helper xs (i+1)
    ...
    return nodes where 
        ...
        createNodes (xs:xss) assocs = Node {up = -1, itm = -1, down = -1}: createNode xs assocs ++ createNodes xss assocs where --spacer 
            createNode xs assocs = map (\x -> Node {up = 0, itm = findTop x assocs, down = 0}) xs where 
                findTop elem [] = -1
                findTop elem ((i,x):xs)  = if elem == (value x) && i/=0 then i else  -- i /= 0 because the first array elem is a spacer with value of i=1
                    findTop elem xs
...
--function like filter, but gives back indices instead of STArray 
...
getValue :: STArray s Int (Item a) -> Int -> ST s a
getValue items i = do 
