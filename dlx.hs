{-
This module contains Algorithm 1, which is described in detail in Chapter 3.1 and 
expanded upon in Chapter 4.1 where the process how to make DLX accept 
secondary items is described
-}
...
{-
Description: Finds all possible solution to an exact cover problem (Algorithm 1)
Input: (primary items, secondary items), options
Output: all possible solutions (each solution is made out of options that solve the problem)
-}
dlx :: Eq a => ([a],[a]) -> [[a]] -> [[[a]]]
...
{-
Description: covers item i and options containing i (Chapter 3.1 Procedure (3.3))
Input: itemArray, nodeArray, index of item i
Output: ST s ()
-}
...
{-
Description: uncovers item i and options containing i (Chapter 3.1 Procedure (3.5))
Input: itemArray, nodeArray, index of item i
Output: ST s ()
-}
...
{-
Description: Converts the solution that consists of respectively 1 elem of each option
  to whole options (Algorithm 4 Chapter 4.1.2)
Input: itemArray, nodeArray, solutions (1 elem of each options)
Output: solutions (whole options)
-}
...
test = runSTArray (do
  ...
  nodeArray [["c","e"],["a","d","g"],["b","c","f"],["a","d","f"],["b","g"],["d","e","g"]] items)
