...
getSymbolicFormat conds board inputs = do
    ...
    let customNumber = 1
    options <- getSymbolicFormatHelper conds board inputs (map (:[]) (concat (boardPositions board 0))) customNumber
    return (filter (\x -> length conds +1 == length x) (concat options)) where

        getSymbolicFormatHelper [] _ _ format customNumber = return format
        getSymbolicFormatHelper (c:conds) board inputs format customNumber = do
            -- c:=6 is the custom condition. It can be called multiple times, so i need different names for each one
            if c /= 6 then do
                options <- getSymbolicOptions c board inputs customNumber
                getSymbolicFormatHelper conds board inputs (combine format (removePos options)) customNumber
            else do 
                options <- getSymbolicOptions c board inputs (customNumber+1)
                getSymbolicFormatHelper conds board inputs (combine format (removePos options)) (customNumber+1)

        removePos :: [[[String]]] -> [[String]]
        removePos = map (foldr (\x r -> tail x ++ r) [])

getSymbolicOptions :: Int -> [[String]] -> [String] -> Int -> IO [[[String]]]
getSymbolicOptions cond board inputs customNumber =
    case cond of
        1 -> rowCond where
            rowCond = do
                ...
                then getConfirmation 1 board (map (1,) inputs) (getOptionRow board (map (1,) inputs) (boardPositions board 0) "r")
                else do
                    putStrLn "Tell me how often an Input appears (at most) per row"
                    inputAppearence <- getInputAppearence inputs "row"
                    getConfirmation 1 board inputAppearence (getOptionRow board inputAppearence (boardPositions board 0) "r")
        ...
        2 -> colCond where
            colCond = do
                ...
                if 'y' == toLower (head oneInput)
                then do
                    options <- getConfirmation 2 board (map (1,) inputs) (getOptionRow (transpose board) (map (1,) inputs) ( transpose (boardPositions board 0)) "c")
                    ...
                    options <- getConfirmation 2 board inputAppearence (getOptionRow (transpose board) inputAppearence ( transpose (boardPositions board 0)) "c")
                    return (sort options (boardPositions board 0))
            ...
            lrDiagCond = do
                ...
                then do
                    options <- getConfirmation 3 board (map (1,) inputs) (getOptionRow (diagonals (reverse board)) (map (1,) inputs)
                        (diagonals (reverse (boardPositions board 0))) "ld" )
                    return (sort options (boardPositions board 0))
                else do
            ...
            rlDiagCond = do
                ...
                then do
                    options <- getConfirmation 4 board (map (1,) inputs) (getOptionRow (diagonals board) (map (1,) inputs)
                        (diagonals (boardPositions board 0)) "rd")
                    return (sort options (boardPositions board 0))
                else do
                    putStrLn "Tell me how often an Input appears (at most) per diagonal"
                    inputAppearence <- getInputAppearence inputs "diagonal"
            ...
            boxCond = do
                ...
                then do
                    options <- getConfirmation 5 board (map (1,) inputs) (getOptionRow (getBoxs board width height) (map (1,) inputs)
                        (getBoxs (boardPositions board 0) width height) "b")
                    return (sort options (boardPositions board 0))
                else do
            ...
            customCond = do
                ...
                then do
                    options <- getConfirmation 6 board (map (1,) inputs) (getOptionRow (shapeRows board) (map (1,) inputs)
                        (shapeRows (boardPositions board 0)) ("c" ++ show customNumber) )
                    return (sort options (boardPositions board 0))
                else do
    ...
    getConfirmation i board inputs formatFunc = do
        ...
        else getSymbolicOptions i board (map snd inputs) customNumber
        ...
        combine (map (:[]) (concat pos)) (concatItems options [])
            ...
            oneInputFormat board (i,input) = let inputAmount = map (\xs -> i - length (filter (==input) xs)) board in
                let itemDigits = scanl (+) 0 inputAmount in
                    concatMap (\(j,xs) -> map (\x -> if not (null x) then [] else map (\k -> lt ++ show k ++ "-" ++ input)
                    [(itemDigits!!j)..(itemDigits!!(j+1)-1)]) xs) (zip [0..] board)
...
getItems opts i = let options = map nub (transpose  opts) in
    let (pos, rest) = splitAt 1 options in
    let (pr, sr) = splitAt i rest in
        (concat pr, concat (sr++pos))
